    ...
    def _update_local_arp_table(self) -> ProtectionResult:
        ...
        try:
            if sys.platform.startswith("linux"):
                ...
                # Classic `arp -d` / `arp -s` needs no interface name -
                # Scapy's interface identifier on Windows (a Npcap
                # device path) isn't something Windows CLI tools accept
                # directly, so we deliberately avoid needing it here.
                subprocess.run(
                    ["arp", "-d", self.baseline.gateway_ip],
                    check=False, capture_output=True,
                )
                result = subprocess.run(
                    ["arp", "-s", self.baseline.gateway_ip, self.baseline.gateway_mac],
                ...
                return ProtectionResult(
                    success=False,
                    action="restore_local_arp_table",
                    detail=f"arp -s failed: {result.stderr.strip() or result.stdout.strip()} "
                           f"(make sure this is running as Administrator)",
                )
                ...
                return ProtectionResult(
                    success=False,
                    action="restore_local_arp_table",
                    detail="Unsupported OS for this prototype (only Linux and Windows are handled).",
                )
